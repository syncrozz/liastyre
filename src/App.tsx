...
export default function App() {
  ...
  // Import Bulk Tyres from CSV / Excel
  const handleImportBulkTyres = (importedTyres: Tire[]) => {
    setTyres((prev) => {
      const updated = [...prev];
  ...
  // Add New Tire record
  const handleAddTyre = (newTire: Tire) => {
    setTyres((prev) => [newTire, ...prev]);
  };

  // Update Stock
  const handleUpdateStock = (tireId: string, newStock: number) => {
    setTyres((prev) =>
      prev.map((t) => (t.id === tireId ? { ...t, storeStock: newStock, totalStock: newStock } : t))
    );
  };

  // Sync Master Data Stock
  const handleSyncMasterStock = (mode: "standard" | "popular" | "reset" | "custom", customQty: number = 10) => {
    setTyres((prev) =>
      prev.map((t) => {
        ...
        if (mode === "reset") {
          ...
          // Top selling sizes get 24 units, others get 8 units
  ...
  const handleDeductStock = (itemsToDeduct: { tireId: string; quantity: number }[]) => {
    setTyres((prev) =>
      prev.map((t) => {
        const match = itemsToDeduct.find((item) => item.tireId === t.id);
        if (match) {
          const newStock = Math.max(0, t.storeStock - match.quantity);
          return {
            ...t,
            storeStock: newStock,
            totalStock: newStock,
